...
export const customEncounterParams =
  'custom:(encounterDatetime,uuid,location:(uuid,name),encounterType:(uuid),obs:(value:(uuid),groupMembers:(value:(uuid),concept:(datatype:(name),uuid),uuid),concept:(datatype:(name),uuid),uuid),patient:(uuid),encounterProviders:(uuid,encounterRole:(uuid),provider:(uuid)))';
